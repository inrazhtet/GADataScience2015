...
api = b.find(name='ul', attrs ={'id':'api'}).find_all(name='li')
for tag in api:
    print tag.text
...
b.find(name='span', attrs={'class' :'itemprop', 'itemprop':'name'})
# get the title
...
b.find(name='meta', attrs={'itemprop': 'contentRating','content' : 'R'}).text
b.find(name='meta', attrs={'itemprop': 'contentRating'})['content']

# get the duration in minutes (as an integer)
b.find(name='time', attrs={'itemprop' : 'duration', 'datetime' : 'PT142M'}).text
